...
public class MyLSVM18Agent extends AbsLSVM18Agent implements IAgent {
	...
	public MyLSVM18Agent(String name) {
		...
		this.avgPrice.put(30./18,16.8);
		this.avgPrice.put(35./18,19.6);
		this.avgPrice.put(40./18,20.6);
		this.avgPrice.put(45./18,18.2);
		this.avgPrice.put(50./18,21.7);
		this.avgPrice.put(55./18,22.4);
	}
	...
	protected Map<String, Double> getBids(Map<String, Double> minBids) {
		...
		if (this.getCurrentRound()==0) {
			for (String s : getProximity()) {
				bids.put(s, this.getValuation(s)*1);
			}
		}
		else {
			...
			if (this.isNationalBidder()) {
				for (String s : getProximity()) if (!myGoods.contains(s)) {
